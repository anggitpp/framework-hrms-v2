    ...
    <div class="card">
        <form method="POST" id="form-edit" action="{{ empty($correction) ? route(str_replace('/', '.', $menu_path).'.store') : route(str_replace('/', '.', $menu_path).'.update', $correction->id) }}" enctype="multipart/form-data">
            ...
            @if(!empty($correction))
                @method('PATCH')
            @endif
            <x-form.header title="{{ empty($correction) ? __('Tambah Data Koreksi') : __('Edit Data Koreksi') }}" />
            <div class="separator mt-2 mb-5 d-flex"></div>
            <div class="card-body pt-0">
                <div class="row">
                    <div class="col-md-6">
                        <x-form.input label="Nomor" name="number" value="{{ $correction->number ?? $lastNumber }}" readonly required />
                        <x-form.select label="Pegawai" name="employee_id" option="- Pilih Pegawai -" :datas="$employees" value="{{ $correction->employee_id ?? '' }}" event="getDetailEmployee();getAttendance();" required/>
                        <x-form.input label="Unit" name="unit" value="{{ $correction->employee->position->unit_id ?? '' }}" readonly />
                    </div>
                    <div class="col-md-6">
                        <x-form.datepicker label="Tanggal Pengajuan" name="date" value="{{ $correction->date ?? date('Y-m-d') }}" required />
                        <x-form.input label="Pangkat" name="rank" value="{{ $correction->employee->position->rank_id ?? '' }}" readonly />
                    </div>
                </div>
                <div class="separator mt-2 mb-5 d-flex"></div>
                <div class="row">
...
@section('scripts')
    <script>
        function getDetailEmployee() {
            ...
            $.ajax({
                url: "{{ route('attendances.corrections.employee') }}",
                type: "GET",
                data: {
        ...
        function getAttendance() {
            ...
            if(date !== '' && employee_id !== '') {
                $.ajax({
                    url: "{{ route('attendances.corrections.attendance') }}",
                    type: "GET",
                    data: {
                        employee_id: employee_id,
                        date: date
