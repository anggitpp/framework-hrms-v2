@props(['url_edit' => '', 'url_destroy' => '', 'url_show' => '', 'isModal' => true, 'url_slot' => '', 'icon_slot' => 'fa-solid fa-list', 'isModalSlot' => true])
    ...
    @if($url_slot)
        <a
            ...
            @else
                href="{{ $url_slot }}"
            @endif
            class="btn btn-icon btn-light-info w-30px h-30px me-1 {{ $isModalSlot ? 'btn-modal' : '' }}" data-bs-toggle="modal">
            <i class="{{ $icon_slot }}"></i>
        </a>
    @endif
    @if($url_show)
