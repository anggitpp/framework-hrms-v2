<div class="card">
    <form method="POST" id="form-edit" action="{{ empty($training) ? route(Str::replace('/', '.', $menu_path).'.training.store', $id) : route(Str::replace('/', '.', $menu_path).'.training.update', $training->id) }}" enctype="multipart/form-data">
        @csrf
        @if(!empty($training))
            @method('PATCH')
        @endif
        ...
        <div class="card-body pt-0">
            <input type="hidden" name="employee_id" value="{{ $training->employee_id ?? $id }}"/>
