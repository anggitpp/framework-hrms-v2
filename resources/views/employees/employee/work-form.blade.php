<div class="card">
    <form method="POST" id="form-edit" action="{{ empty($work) ? route(Str::replace('/', '.', $menu_path).'.work.store', $id) : route(Str::replace('/', '.', $menu_path).'.work.update', $work->id) }}" enctype="multipart/form-data">
        @csrf
        @if(!empty($work))
            @method('PATCH')
        @endif
        ...
        <div class="card-body pt-0">
            <input type="hidden" name="employee_id" value="{{ $work->employee_id ?? $id }}"/>
