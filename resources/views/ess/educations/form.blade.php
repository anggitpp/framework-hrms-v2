<div class="card">
    <form method="POST" id="form-edit" action="{{ empty($education) ? route(Str::replace('/', '.', $menu_path).'.store') : route(Str::replace('/', '.', $menu_path).'.update', $education->id) }}" enctype="multipart/form-data">
        @csrf
        @if(!empty($education))
            @method('PATCH')
        @endif
        ...
        <div class="card-body pt-0">
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <x-form.input label="Nilai/IPK" name="score" value="{{ $education->score ?? '' }}" class="w-50 text-end"/>
                </div>
            </div>
